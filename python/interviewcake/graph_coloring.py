...
def color_graph(graph, colors):
    """Graph arrive as a list of nodes
    We could just iterate through list and assign the first available color
    """
    colors = set(colors)
    for node in graph:
        if node in node.neighbors:
            raise ValueError('Self-referencing node %s' % node.label)

        adjacent_colors = set(n.color for n in node.neighbors)
        # set difference is O(n), so really we should just iterate through colors until we find one that's not in adjacent_colors
        available_colors = colors - adjacent_colors
